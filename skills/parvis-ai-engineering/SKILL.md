...
The reader is a principal engineer or a technology executive, so T2's expert register applies, with cost named in latency, spend, blast radius, operational burden, talent scarcity or migration risk.
...
Design reviews, evaluation plans, triage write-ups and readiness assessments file to the workspace under `tech-plans/` with a manifest row and a lifecycle status, never into memory (T11 record).
...
- **Nothing version-sensitive is quoted from memory.** Model prices, context windows, benchmark scores, API capabilities, specification behavior and framework defaults fall under T2's verify rule, and a `[model]` claim still names the version it depends on.
