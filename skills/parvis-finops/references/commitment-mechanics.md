...
- **No lock at all, and no commitment.** One cloud discounts usage automatically for running enough of the month, nothing purchased and no dimension frozen, above roughly a quarter of the billing month, at two published maximum rates that step down by machine family generation and reach nothing on the newest families or on usage already committed ([verified 2026-09-20], Google Cloud sustained use discount documentation). Where it applies the `[R_od]` in section 5 is not list price for a sustained workload, so a break-even against list overstates the commitment, sometimes by most of the headline advantage.
