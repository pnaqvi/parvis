...
*Document 2 of 6 · release 2.3, September 2026 · New machine, new account, update, and removal, all through one script, `install.sh`.*
...
7. **Verification.** All seventeen roster skills present, every retired skill name absent (not only the ones this run removed), both manifests present, and each home its own git repository with at least one commit.
...
Then restart Claude Code and run `/skills`. Seventeen skills should list.
...
6. **The four nested local-only repositories,** one per confidential section, each made its own repository with a seed commit if it is not one already.
   ...
   ```
7. **Verify.** Each of the seventeen roster skills has a `SKILL.md` under `~/.claude/skills/`, none of the twelve retired `infra-platform-*` names remains there, both homes hold a `MANIFEST.md`, and `own_repo` and `has_commit` both succeed for each home. Then restart Claude Code and run `/skills`.
...
1. `/skills` shows seventeen skills.
