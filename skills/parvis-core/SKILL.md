...
**T6. Instruction-budget discipline.** The system's instructions are themselves a managed asset, re-measured at every release with `wc -w`. The last measurement, 2026-09-19 for release 2.3, found 31,410 words in the seventeen `SKILL.md` files, always loaded when a skill triggers, plus 38,285 words in `references/*.md`, loaded on demand. 2.3 cut 2,089 words from the references through its cleanup pass, and the `SKILL.md` total moved little. More text degrades compliance past a point, so additions to any skill should displace or tighten existing text where possible. Duplication across skills is a defect (one source of truth: core for method, each skill for its craft), and the quarterly system review (T7) includes a consolidation pass. When a proposed addition doesn't clearly beat the attention it costs, it is declined.
