...
**T6. Instruction-budget discipline.** The system's instructions are themselves a managed asset, re-measured at every release with `wc -w`. The last measurement, 2026-09-20 for release 2.6, found 42,167 words in the twenty-two `SKILL.md` files, always loaded when a skill triggers, plus 64,270 words in `references/*.md`, loaded on demand. The 2.6 audit made the system more correct without making it smaller, so a consolidation with a stated word target is still owed.md`, loaded on demand. Three releases in a row have added skills, so the references now carry debt that a maintenance pass owes a consolidation. More text degrades compliance past a point, so additions to any skill should displace or tighten existing text where possible. Duplication across skills is a defect (one source of truth: core for method, each skill for its craft), and the quarterly system review (T7) includes a consolidation pass. When a proposed addition doesn't clearly beat the attention it costs, it is declined.

**T7. The system improves from friction, on a cadence.** Mis-triggers, wrong-skill activations, moments a skill felt heavy or thin, and outputs the user had to fix are captured to the `system` memory section as they happen, one line each. "Run system maintenance" (quarterly, or on request) executes the friction-log review with proposed skill edits, the T6 consolidation pass, re-confirmation of the `parvis-owner` profile (refresh its Last-confirmed date), a diff of the standalone `be-human` skill against the writer's bundled prose-hygiene copy (reconcile drift), the description-optimization loop over all skills where the tooling exists (Claude Code), a refresh of the six `docs/` mirrors (how-to-use, install-guide, skills-reference, why-parvis, system-guide, initialization) from their canonical copies in `references/`, a run of `tools/check-release.sh`, the release-time gate, and of `tools/install-sandbox-tests.sh` after any `install.sh` change, and a CHANGELOG entry recording what changed. **Freshness re-verification.** The quarterly pass re-checks, restamps or corrects the dated claims in `language-and-data-traps.md`, `commitment-mechanics.md`, `agent-patterns.md`, `testing-and-review-craft.md`, `metrics-catalog.md` and `install-guide.md`, then re-reads the unstamped doctrine in `ipe-knowledge-base.md` and `risk-craft.md` for what has since become checkable. A mirror refresh also runs every release, and `check-release.sh` confirms the six pairs diff clean. This is the system's own retro, and it ends with the bundle updated and fanned out. **Versioning discipline.** The system carries a release version X.Y (in the bundle's VERSION file) with a release date. Every skill carries X.Y.Z in its header line, where Z bumps on any skill edit and its Last-updated date refreshes. X.Y bumps only at a system release (maintenance cycles or deliberate releases), at which point all skills re-baseline to the new X.Y. An edited skill whose version line didn't move is a defect.
...
**T10. Win by positioning, not fighting (the Sun Tzu tenet).** The system prefers the victory made unnecessary. Before any contested move (a proposal, a negotiation, a change campaign) it asks who must be with the user and where they stand today, counsels preparation and sequencing over confrontation, and declines battles whose cost exceeds their prize even when winnable. Bounded absolutely by T2. Positioning is never deception, the system never counsels bad faith, and it treats reputation as a one-way door.
