...
Relationship management as a discipline, knowing the terrain before any contested move. At senior levels, initiatives succeed or die on stakeholder positioning long before their technical merits are heard, so this skill makes that terrain explicit, remembered and worked deliberately. Client of parvis-memory, section **`stakeholders`** (confidential, `sync: no`, the same rules as the people sections, professional and factual, and the glass test (T3) governs). Core's tenets apply throughout, T10 positioning especially.

## The registry, one file per stakeholder

`sections/stakeholders/registry/<person-slug>.md`, per the full schema in `references/stakeholder-craft.md`. Read it before any registry work. The schema covers the professional dimension (role, THEIR goals and pressures, decision style, risk appetite), motivation and drivers (what actually moves them, ranked and evidenced), a personal dimension bounded by the **glass-registry test**, which is the glass test (T3) (communication preferences, rapport notes, milestones they've shared), the projects they care about with their stake in each (sponsor / beneficiary / threatened / gatekeeper, linked to the group `projects.md` files), dated evidenced stances, a two-way list of what each side owes, their network (who they listen to, who they influence, coalitions), and a dated history log. Tier and cadence target sit in the file header. T2 throughout, so stances come from evidence and inferences are labeled. On scope, the user's own reports belong in people-leader's evidence logs, and this registry is for peers, upward, external and cross-org.
...
Triggered by "map the stakeholders for <initiative>". The map names every stakeholder who can help, hurt or block, positioned on influence × stance. It names the critical path (whose support is necessary, whose opposition is fatal, who moves others), the gaps (high-influence people whose stance is unknown, and an unknown stance on a critical-path stakeholder is the first thing to fix, T10 positioning), and the sequencing (who to engage before whom, sponsors before skeptics, coalitions before committees). Maps live with the initiative's section (for example a resilience program's map in its group section) with registry files as the per-person source.
...
- **Terrain before engagement.** The skill working a contested proposal, negotiation or change initiative asks T10's question itself, "who has to be with you, and where are they today?", and this skill answers it from the registry and the map.
...
This skill counsels preparation, positioning, sequencing and honest persuasion built on understanding what others need, inside T10 positioning's bound, so never deception, manufactured pressure or bad-faith tactics. T2 binds the user's dealings as much as their documents.

## What lives where

The split runs on content rather than format. Anything naming a person and where they stand is a register and stays in memory, registry files and relationship history in `sections/stakeholders/registry/` inside the confidential `stakeholders` section, initiative maps with the initiative's home section pointing at those files, because memory is where the structural protection sits (T3). The workspace takes the engagement document written to be sent or presented, with a manifest row, carrying the plan's conclusion and leaving person-level stance detail in the registers (T11's altitude rule). Stance changes, commitments made to or by stakeholders → dated registry entries plus, where decision-shaped, the relevant section's ledger.
