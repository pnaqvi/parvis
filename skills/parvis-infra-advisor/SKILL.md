...
- **Session naming:** briefs are `sections/infra-advisor/brainstorm/<issue-slug>.md` in the memory home. Never overwrite a prior session.
- **One source of truth:** the template inside this skill is the canonical team definition. If the user still has older standalone copies of the prompt as loose files, suggest deleting them so they don't drift.
